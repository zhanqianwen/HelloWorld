...
{
    /*
    //For Lagrange Interpolation Polynomial 
    if ((mPos > 0) && (X == mX[mPos - 1]))
    {
        return;
    }
    else if ((mPos == 0) && (X == mX[MaxNum - 1]))
    {
        return;
    }
    printf("X=: %f \n", X);
...
double MidiSmoother::LagrangeInter(double X)
{
    /**/
    double ret = 0;
    for (int i = 0; i < mNum; i++)
